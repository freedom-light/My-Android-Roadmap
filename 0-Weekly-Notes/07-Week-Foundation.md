...
#### 顶点着色器
传入顶点着色器的是位于`物体坐标系`上的坐标，顶点着色器所做的工作是将传入的三维物体坐标投影到像素屏幕坐标。
其中包括坐标系的转换(物体坐标系→世界坐标系→摄像机坐标系→屏幕坐标系)，裁剪，映射。
顶点着色器最终将传入的三维物体坐标与屏幕的像素坐标一一对应，并通过预定义的`gl_position`变量向后续处理步骤传递。
#### 片段着色器
#### 内建特殊变量
| 变量 | 描述 |
|------|------|
...
|`gl_Position`|是顶点着色器中**最重要、必须赋值**的一个内置变量，它决定了顶点在裁剪空间中的最终位置。|
#### 内建常量
| 类型 | 名称 | 最小值 | 描述 |
|------|------|--------|------|
...
8. 帧缓冲 (Frame Buffer)
#### 渲染管线坐标系转换流程
1. 局部/模型坐标系：模型的顶点数据最初定义在这个坐标系中，例如你的正方形顶点数据[-1, -1, 0] - [1, 1, 0]
2. 世界坐标：将局部坐标转换到世界坐标，其中包括平移、旋转、缩放等操作，将模型放置在三维世界中的正确位置。
3. 视图/摄像机坐标：通过视图矩阵 (View Matrix) 将世界坐标转换到摄像机（观察者）的视角。
4. 裁剪坐标 (Clip Coordinates): 通过投影矩阵 (Projection Matrix) 将视图坐标转换到裁剪坐标。在这个阶段，顶点会被转换到一个齐次坐标空间，其范围通常是 [-w, w]。
5. NDC坐标：在裁剪坐标之后，会进行透视除法 (Perspective Divide)，将 (x,y,z,w) 转换为 (x/w, y/w, z/w)。此时，所有可见的顶点都将落在 [-1, 1] 的立方体内。这就是NDC坐标。
   * 并且深度信息仍然存在，能感知到物体的前后关心
6. 屏幕/视口坐标：NDC坐标会被映射到实际的屏幕像素坐标上，通过 glViewport 设置的视口大小来决定。
   * 物体的前后关系没有丢失，z信息被记录下来了。
  
只有在NDC坐标系中，你才能正确地利用 MVP 矩阵的逆矩阵，将屏幕上的2D点准确地反向映射到3D世界中的模型（纹理）上。
