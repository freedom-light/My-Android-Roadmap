...
## 更新时需注意事项
假设场景：你给AI提了需求，AI完成了功能。（用户层面的完成了功能，可能**代码\技术栈**并不好）

对应AI反馈的文字，以及写的代码，进行全篇阅读（我认为就算是一个大致的阅读也是OK的）。

例如你发现了一个问题，你指出后，它给了你两种选项，**可以通过..方式临时解决，或者通过..补充整个流程**一定要细心识别，因为这两种方式可能都是错误的（遇到了）。省心一点的方式 可以通过把Agent反馈的方案交给另外一个AI，让它检查是否符合行业实现标准。或如果自己有想法，包括：知道使用什么技术栈，知道怎样实现更好，直接提给AI，越详细越好，少让它随意发挥，也就少了一点偷懒的可能。

**小结：包括现在做的提案也好，定义规则文档也好，本质都是更加详细的告诉AI该怎么做，只不过是把常用的规则沉淀成了文档放入项目里面。**
